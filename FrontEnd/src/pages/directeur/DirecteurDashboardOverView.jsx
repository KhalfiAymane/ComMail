...
const DirecteurDashboardOverview = () => {
  const { userData, setIsModalOpen } = useOutletContext(); // Access setIsModalOpen from layout
  ...
  const statsCards = [
    { title: "Sent", value: stats.sent, icon: <FaUpload />, color: "#A78800", bgColor: "#A78800/10", trend: 8.5 },
    { title: "Received", value: stats.received, icon: <FaDownload />, color: "#3B82F6", bgColor: "#3B82F6/10", trend: 12.3 },
    { title: "Pending", value: stats.pending, icon: <FaClipboardList />, color: "#F59E0B", bgColor: "#F59E0B/10", trend: -4.2 },
    { title: "Archived", value: stats.archived, icon: <FaArchive />, color: "#8B5CF6", bgColor: "#8B5CF6/10", trend: 15.7 }
  ];
  ...
  const recentActivity = [
    ...
    { id: '4', subject: 'Note de Service', receiver: 'All Staff', date: '2025-03-13', status: 'validé', type: 'sent' },
    { id: '5', subject: 'Rapport Mensuel', sender: 'Operations Dept', date: '2025-03-12', status: 'nouveau', type: 'received' },
    { id: '6', subject: 'Plan Stratégique', receiver: 'Direction', date: '2025-03-11', status: 'validé', type: 'sent' },
  ];
  ...
  const notifications = [
    ...
    { id: '3', text: 'Monthly department report due tomorrow', urgent: true, time: '1 hour ago' },
    { id: '4', text: 'Team meeting scheduled for 14:00', urgent: false, time: '3 hours ago' },
  ];
  ...
  const renderHeader = () => (
    <div className="mb-8">
      <div className="p-6 rounded-2xl bg-gradient-to-r from-[#A78800] to-[#8A6D00] shadow-lg">
        <h1 className="text-3xl font-bold mb-2 text-white">Welcome back, {(userData?.fullName || 'Directeur').split(' ')[0]}</h1>
  ...
  const renderStatsGrid = () => (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
      {statsCards.map((card, index) => (
        <div 
          key={index} 
          className={`relative overflow-hidden rounded-2xl group ${darkMode ? 'bg-[#1F2024]' : 'bg-white'} 
            transition-all duration-300 transform hover:translate-y-[-5px] hover:shadow-xl 
            border border-transparent hover:border-${card.color}/50`}
        >
          <div className="p-5">
            <div className="flex justify-between items-center">
              ...
              <div 
                ...
                ${darkMode ? 'bg-[#131313]' : 'bg-gray-100'} group-hover:bg-[${card.color}] transition-colors duration-300`}
                style={{ background: `${darkMode ? '#131313' : '#F9FAFB'}`, color: card.color }}
              >
                {card.icon}
              </div>
            </div>
    ...
    <div className={`rounded-2xl ${darkMode ? 'bg-[#1F2024]' : 'bg-white'} p-6 mb-8`}>
      ...
      <div className="grid grid-cols-4 gap-3">
        <button 
          ...
          className="group flex flex-col items-center justify-center py-4 px-3 rounded-xl bg-[#A78800] text-white 
            transition-all duration-300 hover:bg-[#8A6D00] hover:shadow-lg transform hover:scale-105"
        >
          <FaEnvelope className="text-xl mb-1" />
          <span className="text-sm mt-1">New Message</span>
        </button>
        <a href={`/dashboard/${userData?.department}/inbox`} className={`group flex flex-col items-center justify-center py-4 px-3 rounded-xl 
          ${darkMode ? 'bg-[#131313] hover:bg-[#1F2024]' : 'bg-gray-100 hover:bg-gray-200'}
          transition-all duration-300 hover:shadow-lg transform hover:scale-105
          border border-transparent hover:border-[#3B82F6]`}>
        ...
        <a href={`/dashboard/${userData?.department}/pending`} className={`group flex flex-col items-center justify-center py-4 px-3 rounded-xl 
          ${darkMode ? 'bg-[#131313] hover:bg-[#1F2024]' : 'bg-gray-100 hover:bg-gray-200'}
          transition-all duration-300 hover:shadow-lg transform hover:scale-105
          border border-transparent hover:border-[#F59E0B]`}>
          <FaClipboardList className="text-xl mb-1 text-[#F59E0B]" />
          <span className="text-sm mt-1">Pending</span>
          {stats.pending > 0 && (
            <span className="mt-1 px-2 py-0.5 text-xs bg-[#F59E0B] text-white rounded-full">{stats.pending}</span>
          )}
        </a>
        <a href={`/dashboard/${userData?.department}/archived`} className={`group flex flex-col items-center justify-center py-4 px-3 rounded-xl 
          ${darkMode ? 'bg-[#131313] hover:bg-[#1F2024]' : 'bg-gray-100 hover:bg-gray-200'}
          transition-all duration-300 hover:shadow-lg transform hover:scale-105
          border border-transparent hover:border-[#8B5CF6]`}>
  ...
  const renderActivityTable = () => (
    <div className={`rounded-2xl ${darkMode ? 'bg-[#1F2024]' : 'bg-white'} p-6 mb-8`}>
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {recentActivity.map((item) => (
              <tr 
                key={item.id}
                className={`group border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'} hover:${darkMode ? 'bg-[#131313]/50' : 'bg-gray-50'} transition-colors duration-300`}
              >
      ...
      <a 
        href={`/dashboard/${userData?.department}/activity`} 
  ...
  const renderNotifications = () => (
    <div className={`rounded-2xl ${darkMode ? 'bg-[#1F2024]' : 'bg-white'} p-6 mb-8`}>
        ...
        {notifications.map((note) => (
          ...
          >
            <div className="flex items-start">
              ...
              {note.urgent && (
                <a 
                  href={`/dashboard/${userData?.department}/pending`}
      ...
      <a 
        href={`/dashboard/${userData?.department}/notifications`} 
